...
separators = ['<num_sep>', '<sc_sep>', '<cc_sep>', '<us_sep>']
capital_letter_token = '<capital>'
...
def get_curr_seq(seq, ps, current_target, current_p, text_field):
    ...
    if current_target_word == capital_letter_token:
        return None, None, seq, ps

    if (current_target_word not in separators) and (last_seq_word not in separators):
        return seq, ps, [current_target], [current_p]
    elif (current_target_word in separators) != (last_seq_word in separators):
        # ongoing split word
        return None, None, seq + [current_target], ps + [current_p]
    else:
        # current_target in separators and seq[-1] not separators
        raise AssertionError(f"Two separators in a row are not: {last_seq_word}, {current_target_word}")


sep_start_new = '`s'
sep_end_new = 's`'
capital_letter_token_new = '`C'


def get_curr_seq_new(seq, ps, current_target, current_p, text_field):
    current_target_word = text_field.vocab.itos[current_target[0]]
    if current_target_word == capital_letter_token_new:
        return None, None, seq, ps

    if not seq:
        if current_target_word == sep_start_new:
            return None, None, [current_target], [current_p]
        elif current_target_word == sep_end_new:
            raise AssertionError(f"Split end separator without split start")
        else:
            return [current_target], [current_p], [], []
    else:
        if current_target_word == sep_start_new:
            raise AssertionError(f"Second split start separator")
        elif current_target_word == sep_end_new:
...
def validate_with_cache(stepper, dl, metrics, text_field, theta=2, lambdah=0.0, window=1000):
    ...
    with no_grad_context():
        ...
        for (*x, y) in iter(dl):
            ...
            for idx, vocab_loss in enumerate(softmax_output_flat):
                ...
                curr_seq, current_ps, seq, ps = get_curr_seq(seq, ps, current_target, current_p, text_field)
                if curr_seq is not None:
                    for pp in current_ps:
                        loss += (-torch.log(pp)).data[0]
                    seqs_in_batch += 1
                    if len(current_ps) > 0:
                        logging.info("".join(map(lambda x: text_field.vocab.itos[x[0]], curr_seq)))
                        lss = list(map(lambda x: (-torch.log(x)).data[0], current_ps))
                        logging.info("" + str(list(map(lambda x: f'{x:.3f}', lss))) + f'= {sum(lss):.3f}')
                        logging.info("============================")
